...
var instance = new Razorpay({
    key_id: 'rzp_test_FiyquSjN1tclr2',
    key_secret: 'D7LEjubqUlKIvGSwKyTB7kMi',
  });
...
module.exports = {
        ...
        return new Promise(async(resolve, reject) => {
            let total = await db.get().collection(collection.CART_COLLECTION).aggregate([
                ...
                },
                {
                    $group:{
                        _id:null,
                        total:{$sum:{$multiply:['$quantity',10000]}}
                    }
                }
            ]).toArray()
    ...
    editProfile:(userId, userDetails) => {
        return new Promise(async(resolve, reject) => {
            ...
            if(user) {
                bcrypt.compare(userDetails.cPassword, user.Password).then(async(status) => {
                    if(status) {
